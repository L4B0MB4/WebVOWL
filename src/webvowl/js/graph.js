...
module.exports = function (graphContainerSelector) {
  ...
  function redrawContent() {
    ...
    linkGroups = linkContainer.selectAll(".link").data(links).enter().append("g").classed("link", true);
          ...
          if (node.property) {
            ...
            if (node.property().inverse) {
              ...
              if (name === searchString) computeDistanceToCenter(node);
                ...
                if (node.property().inverse()) {
                  if (!node.property().inverse().getHalos()) node.property().inverse().drawHalo();
  ...
  function computeDistanceToCenter(node, inverse) {
    ...
    if (node.property && highlightOfInv === true) {
      if (node.property().inverse()) {
        ...
        if (node.property().getHalos()) rectHalo = node.property().getHalos().select("rect");
        else {
  ...
  var updateTargetElement = function () {
    ...
    var w = graph.options().width();
    if (graph.options().leftSidebar().isSidebarVisible() === true) w -= 200;
    var h = graph.options().height();
    ...
    if (graph.options().leftSidebar().isSidebarVisible() === true) cx += 200;
    var cp = getWorldPosFromScreen(cx, cy, graphTranslation, zoomFactor);
